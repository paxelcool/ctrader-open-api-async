...
class AsyncTcpProtocol:
    ...
    async def send(self, message: Any, instant: bool = False, client_msg_id: Optional[str] = None, 
                   is_canceled: Optional[Callable] = None) -> None:
    ...
    async def _serialize_message(self, message: Any, client_msg_id: Optional[str] = None) -> bytes:
            ...
            while self.is_connected:
                ...
                if not self._send_queue:
                    ...
                    if (self._last_send_time is None or 
                        (datetime.now() - self._last_send_time).total_seconds() > 20):
